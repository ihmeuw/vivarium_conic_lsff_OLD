...
class IronDeficiencyAnemia(Risk):
    ...
    def setup(self, builder):
        ...
        self.anemia_thresholds = builder.lookup.build_table(
            ...
            value_columns=['mild_threshold', 'moderate_threshold', 'severe_threshold']
        )
...
def get_anemia_thresholds():
    ...
    """
    return pd.DataFrame({'age_start': [0, 1 / 12, 5],
                         'age_end': [1 / 12, 5, 15],
...
def get_iron_deficiency_disability_weight(builder):
    ...
    sequelae = ['mild', 'moderate', 'severe']
    seq_dw = []
    for seq in sequelae:
        df = builder.data.load(f'sequela.{seq}_iron_deficiency_anemia.disability_weight')
